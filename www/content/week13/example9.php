...
$query = "SELECT id, album, artist, song FROM song WHERE album LIKE 'Led Zepplin 4' ";
...
foreach ($idArray as $value) {
	$query = "UPDATE song SET album = 'Led Zeppelin IV' WHERE id = " . $value;
	$result = mysqli_query($conn1, $query);
}
...
$query = "SELECT id, album, artist  FROM song WHERE album = 'Led Zeppelin IV'";
$result = mysqli_query($conn1, $query);
echo "<table class='table table-striped table-bordered table-hover'>";
echo "<thead><tr><th>ID</th><th>Album</th><th>Artist</th></tr></thead>";
echo "<tbody>";
